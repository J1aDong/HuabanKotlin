...
import com.j1adong.huabankotlin.mvp.model.Api;
...
/**
 * Created by jess on 8/5/16 11:07 contact with jess.yan.effort@gmail.com
 */
public class WEApplication extends BaseApplication
{
	...
	{
		super.onCreate();
		mAppComponent = DaggerAppComponent.builder().appModule(getAppModule())
				.clientModule(getClientModule()).imageModule(getImageModule())
				.globeConfigModule(getGlobeConfigModule()) // 全局配置
				.serviceModule(new ServiceModule()) // 自己创建
				.cacheModule(new CacheModule()) // 自己创建
				.build();
	...
	{
		WEApplication application = (WEApplication) context
		...
		return GlobeConfigModule.buidler().baseurl(Api.APP_BASE_URL)
				...
				{// 这里可以提供一个全局处理http响应结果的处理类,
					...
					public Response onHttpResultResponse(String httpResult,
							Interceptor.Chain chain, Response response)
					{
						...
						// create a new request and modify it accordingly using
						// the new token
						// Request newRequest =
						// chain.request().newBuilder().header("token",
						// newToken)
						// .build();
					...
					@Override
					public Request onHttpRequestBefore(Interceptor.Chain chain,
							Request request)
					{
						// 如果需要再请求服务器之前做一些操作,则重新返回一个做过操作的的requeat如增加header,不做操作则返回request

						// return chain.request().newBuilder().header("token",
						// tokenId)
						// .build();
						return request;
					}
				}).responseErroListener(new ResponseErroListener()
				{
					...
					@Override
					public void handleResponseError(Context context,
							Exception e)
					{
